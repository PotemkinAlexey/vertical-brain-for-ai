...
def test_trigger_reindexing_populates_cache(tmp_path):
    ...
    chunk_a = store.save_chunk(Chunk(node_path="WORK/A", content="fact one"))
    chunk_b = store.save_chunk(Chunk(node_path="WORK/B", content="fact two"))
    es = EmbeddingSearch(store, MockEmbeddingProvider("m1"), validate_schema=False)

    count = es.trigger_reindexing()

    assert count == 2
    assert store.get_vector(chunk_a.content_hash, "m1") is not None
    assert store.get_vector(chunk_b.content_hash, "m1") is not None
...
def test_trigger_reindexing_scopes_to_node_path(tmp_path):
    store = JsonStore(tmp_path)
    chunk_in = store.save_chunk(Chunk(node_path="WORK/A", content="in scope"))
    chunk_out = store.save_chunk(Chunk(node_path="OTHER/B", content="out of scope"))
    es = EmbeddingSearch(store, MockEmbeddingProvider("m1"), validate_schema=False)

    count = es.trigger_reindexing(node_path="WORK")

    assert count == 1
    assert store.get_vector(chunk_in.content_hash, "m1") is not None
    assert store.get_vector(chunk_out.content_hash, "m1") is None
...
def test_trigger_reindexing_with_new_provider_clears_old_cache(tmp_path):
    store = JsonStore(tmp_path)
    chunk = store.save_chunk(Chunk(node_path="WORK/A", content="fact"))
    provider_v1 = MockEmbeddingProvider("model-v1")
    store.set_vector(chunk.content_hash, "model-v1", [0.1, 0.2])

    es = EmbeddingSearch(store, provider_v1, validate_schema=False)
    provider_v2 = MockEmbeddingProvider("model-v2")
    es.trigger_reindexing(provider_v2)

    assert store.get_vector(chunk.content_hash, "model-v1") is None
    assert store.get_vector(chunk.content_hash, "model-v2") is not None
