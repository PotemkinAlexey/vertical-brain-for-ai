...
def test_vacuum_skips_immutable_chunks_by_default(tmp_path):
    store = SQLiteStore(tmp_path)
...
def test_vacuum_can_include_immutable_chunks_with_force(tmp_path):
    store = SQLiteStore(tmp_path)
    chunk = Chunk(node_path="WORK/A", content="immutable spec", layer="bronze",
                  status="stale", immutable=True)
    store.ensure_node("WORK/A")
    ...
    result = store.vacuum(retention_hours=0, dry_run=False, force=True, include_immutable=True)
